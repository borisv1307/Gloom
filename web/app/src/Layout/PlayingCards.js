        ...
        if (cardsCreated == false) {
            for (let i = 0; i < selectedImages.length; i++) {
                ...
                this.state.cards.push({
                    id: currentId,
                    cardName: image[currentId].cardName,
                    cardType: "cardsInHand",
                    backgroundImage: "url(" + image[currentId].src + ")",
                    imgSrc: image[currentId].src
                });
                this.state.cardsInitial.push({
                    id: currentId,
                    cardName: image[currentId].cardName,
                    cardType: "cardsInHand",
                    backgroundImage: "url(" + image[currentId].src + ")"
    ...
    render() {
        ...
        return (
            <div>
                <div className="shortrest">
                    {
                        ...
                        <Popup trigger={<button disabled={cards.discardPile.length < 2} id="shortrestbutton"
                                                onClick={() => {
                            ...
                            {close => (
                                <div className="modal">
                                    ...
                                    <div className="card-container">
                                        <button id="GetCard" onClick={() => {
                                            this.onClickMoveToShortRest(cards.discardPile[Math.floor(Math.random() * cards.discardPile.length)])
                                        }}>
                                        ...
                                        <br/>
                                        <br/>
                                        <button id="sendToLost" onClick={() => {
                                            this.onClickSendToLost(cards.cardsInShortRest);
                                            close();
                                        }}>
                                            {this.state.isToggleOn ? 'Send to Lost' : 'Continue'}
                                        </button>
                                        <button id="reDraw" onClick={() => {
                                            this.onClickReDraw(cards.discardPile);
                                        }}>
                                            Redraw
                                        </button>
                                    </div>
                ...
                <div id="CardsInHand">
                    <div className="cards-in-hand-pile"
                         style={{display: "flex", justifyContent: "space-evenly", position: "absolute", bottom: 0}}
                         onDragOver={(event) => this.onDragOver(event)}
                         onDrop={(event) => {
                             this.onDrop(event, "cardsInHand")
                         }}>
