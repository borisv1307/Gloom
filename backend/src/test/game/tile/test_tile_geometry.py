...
def test_center_tunnel_on_top_of_burrow_on_waypoint_a_causes_waypoints_to_match_coordinates(waypoint_b):
    room_one = Burrow()
    room_two = Tunnel()

    moved_room = TileGeometry.center_room_a_on_room_b_by_waypoint(room_one, room_two, waypoint_b)

    exit_waypoint = waypoint_b.get_exit(room_one)
    entry_waypoint = waypoint_b.get_entrance(moved_room)
...
def test_overlay_room_b_on_room_a_returns_room_b_after_rotation(waypoint_b):
    room_one = Burrow()
    room_two = Tunnel()

    moved_room = TileGeometry.overlay_room_a_on_room_b(room_one, room_two, waypoint_b)
